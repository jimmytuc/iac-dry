dependencies {
  paths = ["../network", "../ssm-mysql", "../eks-cluster"]
}
...
inputs = {
  cluster_identifier      = "${include.root.locals.environment}-mysql"
  master_dbname           = "root"
  master_username         = "passwd"
  master_password         = dependency.ssm_parameters_mysql.outputs.value
